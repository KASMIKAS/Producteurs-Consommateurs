...
class Buffer:
    def __init__(self, nb_cases):
        ...
        self.msg_dispo = [Condition(self.lock), Condition(self.lock)]

    def produce(self, msg_val, msg_type, msg_source):
        with self.lock:
            while(self.nb_elts.value==self.nb_cases):
            ...
            self.msg_dispo[msg_type].notify()


    def consume(self, id_cons, m_type):
        with self.lock:
            while(self.nb_elts.value==0 or self.storage_type[self.ptr_cons.value] != m_type):
                self.msg_dispo[m_type].wait()
            ...
            if(self.nb_elts.value!=0):
                self.msg_dispo[self.storage_type[self.ptr_cons.value]].notify()
...
def consumer(id_cons, nb_times, buffer, m_type):
    for _ in range(nb_times):
        time.sleep(.5 + random.random())
        buffer.consume(id_cons, m_type)
...
if __name__ == '__main__':
    ...
    for id_cons in range(nb_cons):
        ...
        cons=Process(target=consumer, args=(id_cons, nb_times_cons, buffer, m_type))
        cons.start()
